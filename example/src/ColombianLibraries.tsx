...
export default class ColombianLibraries extends React.Component<{}, State> {
      ...
      .then(res => this.setState(
            {
                data: res.map(item => ({
                    ...
                    latitude: Number(item.georeferencia.replace(/[()]/g, '').split(',')[0].replace('°', '')),
                    longitude: Number(item.georeferencia.replace(/[()]/g, '').split(',')[1].replace('°', '')),
                }))
            }));
  }
